...
SELECT product_id, list_price, RANK() OVER( ORDER BY list_price DESC) as rank_high_to_low
...
SELECT product_id, list_price, RANK() OVER( ORDER BY list_price) as rank_high_to_low
FROM `nod-sql-copy.bike_stores.production_products`;
...
SELECT c.category_name  
    , p.brand_id
    , list_price
    , AVG(list_price) OVER(PARTITION BY p.brand_id) AS brand_avg_price
FROM `nod-sql-copy.bike_stores.production_categories` as c
JOIN `nod-sql-copy.bike_stores.production_products` as p
...
-------------------------------------------------------------------------------
SELECT *,
  sum(order_count) OVER(ORDER BY month) AS order_count_comulative_sum
FROM(
...
*/
-------------------------------------------------------------------------------
SELECT month, ROUND((order_count - prev_order_count) / prev_order_count * 100)
FROM(
  SELECT month, order_count, LAG(order_count) OVER(ORDER BY month) AS prev_order_count
  FROM(  
  SELECT COUNT(*) as order_count 
      , DATE_TRUNC(order_date, month) as month 
    FROM `nod-sql-copy.bike_stores.orders`
...
-------------------------------------------------------------------------------
SELECT *,
  AVG(order_count) OVER(ORDER BY day) as daily_moving_avg,
  AVG(order_count) OVER(ORDER BY week) as weekly_moving_avg,
FROM(    
SELECT COUNT(*) as order_count, 
